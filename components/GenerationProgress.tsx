...
const labels: Record<"en" | "zh", Record<Step, string>> = {
  en: { validating: "Validate", analyzing: "Analyze", generating: "Generate", verifying: "Verify", complete: "Finish" },
  zh: { validating: "校验", analyzing: "分析", generating: "生成", verifying: "核对", complete: "完成" },
};

/** `correcting` is a sub-phase of verification; show it on the same step. */
function stepIndex(stage: GenerationStage | null) {
  if (!stage) return -1;
  return steps.indexOf(stage === "correcting" ? "verifying" : stage);
}
  ...
  return <div className="gen-steps-wrap">
    <ol className="gen-steps" aria-label={zh ? "生成阶段" : "Generation stages"}>
      {steps.map((step, index) => {
        const state: StepState = done || index < current ? "done" : index === current ? (failed ? "error" : "active") : "pending";
        return <li key={step} className="gen-step" data-state={state} aria-current={state === "active" ? "step" : undefined}>
    ...
    {elapsed !== null && running && !failed && <div className="gen-progress-meta" aria-hidden="true">
      ...
      <span>{zh ? "通常需要 2–4 分钟" : "Usually takes 2–4 minutes"}</span>
    </div>}
  </div>;
}
