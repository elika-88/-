...
import { adminReady, adminSetupIssue, adminTotpEnabled, confirmAdminTotp, loginAdmin, logoutAdmin, readStoredSettings, saveStoredSettings, validAdminSession, withAdminDb, type AdminLoginResult } from '@/lib/server/admin-db';
import { ApiBaseUrlSchema, DEFAULT_API_BASE_URL, DEFAULT_MODEL } from '@/lib/provider';
export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';
const cookieName = 'lumina_admin';
const enrollmentCookie = 'lumina_admin_enrollment';
...
export async function GET(request: NextRequest) {
  try {
    if (!await authorized(request)) return json({ authenticated: false, configured: adminReady(), setupError: adminSetupIssue(), totpEnabled: await adminTotpEnabled() }, 401);
...
function loginResponse(request: NextRequest, result: AdminLoginResult) {
  if ('error' in result) {
    const messages = {
      ...
      MFA_REQUIRED: 'Enter the six-digit code from Google Authenticator.',
      INVALID_CODE: 'The authenticator code is invalid or already used. Wait for a new code.',
      ENROLLMENT_EXPIRED: 'Authenticator setup expired. Sign in again to restart setup.',
    };
  ...
  if ('enrollment' in result) {
    const response = json({ authenticated: false, enrollment: result.enrollment });
    response.cookies.set(enrollmentCookie, result.enrollmentToken, { ...options, maxAge: 300 });
    return response;
  }
  const response = json({ authenticated: true });
  response.cookies.set(cookieName, result.token, { ...options, maxAge: 8 * 3600 });
  response.cookies.set(enrollmentCookie, '', { ...options, maxAge: 0 });
  return response;
}
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (body.action === 'login') {
      ...
      if (body.code !== undefined && (typeof body.code !== 'string' || body.code.length > 6)) return json({ error: 'Invalid authenticator code.' }, 400);
      return loginResponse(request, await loginAdmin(body.password, body.code));
    }
    if (body.action === 'confirm_totp') {
      if (typeof body.code !== 'string' || !/^\d{6}$/.test(body.code)) return json({ error: 'Enter a six-digit authenticator code.' }, 400);
      return loginResponse(request, await confirmAdminTotp(request.cookies.get(enrollmentCookie)?.value ?? '', body.code));
    }
    ...
    if (body.action === 'logout') {
      ...
      response.cookies.set(enrollmentCookie, '', { path: '/api/admin', maxAge: 0 });
      return response;
    }
